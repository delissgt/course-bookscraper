...
class ScrapeOpsFakeUserAgentMiddleware:
    ...
    def __init__(self, settings):
        ...
        self.scrapeops_fake_user_agents_active = settings.get('SCRAPEOPS_FAKE_USER_AGENT', False)
...
class MyProxyMiddleware(object):
    ...
    def process_request(self, request, spider):
        user_credentials = '{user}:{passwd}'.format(user=self.user, passwd=self.password)
        basic_authentication = 'Basic ' + base64.b64encode(user_credentials.encode()).decode()
        host = 'http://{endpoint}:{post}'.format(endpoint=self.endpoint, port=self.port)
